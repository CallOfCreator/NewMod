...
public static class PendingEffectManager
{
    ...
    /// </summary>
    public static HashSet<PlayerControl> pendingEffects = new();
    ...
    {
        if (target != null && !pendingEffects.Contains(target)) pendingEffects.Add(target);
    }
    ...
    {
        if (target != null && pendingEffects.Contains(target)) pendingEffects.Remove(target);
    }

    /// <summary>
    ///     Applies all pending effects to their respective players and then clears the lists of pending effects and waiting
    ///     players.
    /// </summary>
