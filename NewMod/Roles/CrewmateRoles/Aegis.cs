...
public class Aegis : CrewmateRole, INewModRole
{
    ...
    public static void OnAnyButtonClick(MiraButtonClickEvent evt)
    {
        ...
        var block = ShieldArea.IsInsideOthersWard(lp);
    ...
    public static void OnBeforeMurder(BeforeMurderEvent evt)
    {
        ...
        foreach (var area in ShieldArea.AreasAt(evt.Target.GetTruePosition()))
        {
            ...
            if (aegis.AmOwner)
                Coroutines.Start(CoroutinesHelper.CoNotify("<color=#3A9EFF>Aegis Ward Alert:</color> Kill attempt blocked inside your ward!"));
        }
    }
        ...
        foreach (var area in ShieldArea.AreasAt(pos))
        {
            ...
            switch (ShieldArea.Mode)
            {
                case AegisOptions.AegisMode.WarnOnly:
                    if (owner.AmOwner)
                        Coroutines.Start(CoroutinesHelper.CoNotify("<color=#3A9EFF>A kill happened in your ward</color>"));
                    break;
                ...
                case AegisOptions.AegisMode.BlockAndReveal:
                    if (owner.AmOwner)
                    {
                        ...
                        Coroutines.Start(CoroutinesHelper.CoNotify($"<color=#3A9EFF>Aegis Ward Alert:</color> A player was killed inside your ward by <color=#FF4444><b>{killerName}</b></color>!"));
                    }
