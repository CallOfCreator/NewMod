...
public class DebugWindow(nint ptr) : MonoBehaviour(ptr)
{
    ...
    public readonly DragWindow DebuggingWindow = new(new Rect(10, 10, 0, 0), "NewMod Debug Window", () =>
    {
        ...
        if (Instance.tab == 0)
        {
            ...
            var force = GUILayout.Toggle(NewMod.ForceEnableAllSeasons.Value, "Force All Seasons");
            if (force != NewMod.ForceEnableAllSeasons.Value) NewMod.ForceEnableAllSeasons.Value = force;

            if (GUILayout.Button("Become Necromancer") && allow) PlayerControl.LocalPlayer.RpcSetRole((RoleTypes)RoleId.Get<NecromancerRole>());
    ...
    [HideFromIl2Cpp] public bool EnableDebugger { get; set; }
