...
public static class BeaconShowMapPatch
{
    ...
    public static void Postfix(MapBehaviour __instance, MapOptions opts)
    {
        ...
        if (!__instance || !__instance.IsOpen)
        {
            _armedPulseThisOpen = false;
            return;
        }
        ...
        Helpers.CreateAndShowNotification($"Beacon pulse active ({BC.charges}/{OptionGroupSingleton<BeaconOptions>.Instance.MaxCharges} left)", new Color(0.75f, 0.65f, 1f), spr: NewModAsset.RadarIcon.LoadAsset());
    }
    ...
    public static IEnumerator CoUpdateMarkers(MapBehaviour map)
    {
        while (Time.time < BC.pulseUntil && map && map.IsOpen && ShipStatus.Instance)
        {
            ...
            for (var i = 0; i < players.Length && i < _markers.Count; i++)
            {
                var pc = players[i];
                var mrk = _markers[i];
                if (!pc || !mrk) continue;

                var v = pc.transform.position;
    ...
    {
        private static void Postfix(MapCountOverlay __instance)
        {
            if (PlayerControl.LocalPlayer.Data.Role is not BC) return;
