...
/// </summary>
public class AssignButton : CustomActionButton, IEnergyAbility
{
    ...
    protected override void OnClick()
    {
        ...
        menu.Begin(player => !player.Data.IsDead && !player.Data.Disconnected && player.PlayerId != PlayerControl.LocalPlayer.PlayerId, player =>
        {
            ...
            var missions = ((MissionType[])Enum.GetValues(typeof(MissionType))).Where(mission => mission != MissionType.DrainEnergy).ToArray();
            ...
            if (mission == MissionType.KillMostWanted)
            {
                ...
                if (candidates.Length == 0)
                    mission = MissionType.CreateFakeBodies;
                else
    ...
    public static IEnumerator CoResetCamera(FollowerCamera cam, float duration)
    {
        ...
        var shouldShake = OptionGroupSingleton<SpecialAgentOptions>.Instance.ShouldShakeCamera;
