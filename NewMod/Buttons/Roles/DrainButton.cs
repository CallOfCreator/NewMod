...
public class DrainButton : CustomActionButton<PlayerControl>
{
    ...
    {
        Target?.cosmetics.SetOutline(active, new Nullable<Color>(Color.magenta));
    }
    ...
    protected override void OnClick()
    {
        ...
        if (PlayerControl.LocalPlayer.AmOwner) HudManager.Instance.Notifier.AddDisconnectMessage($"The Drain effect will be applied to {Target.Data.PlayerName} after the meeting ends.");
    ...
    {
        if (Utils.waitingPlayers.Contains(PlayerControl.LocalPlayer)) return false;
        return base.CanUse();
    }
}