...
public static class SeasonManager
{
    ...
    [
        .. ContentSeasons.SelectMany(season => season.GetSeasonAchievementTabTypes()).Distinct()
    ];
    ...
    public static void InjectSeasonContent()
    {
        ...
        if (roleQueueProperty?.GetValue(instance) is not Dictionary<MiraPluginInfo, List<Type>> roleQueue) return;
        ...
        foreach (var season in ContentSeasons)
        {
            foreach (var type in season.GetSeasonRoleTypes())
            {
                ...
                foreach (var method in AccessTools.GetDeclaredMethods(type))
                {
                    ...
                    if (parameters.Length != 1 || !parameters[0].ParameterType.IsSubclassOf(typeof(MiraEvent))) continue;
            ...
            {
                if (registerModifier == null || !RegisteredSeasonContent.Add(type)) continue;
            ...
            {
                if (registerOptions == null || !RegisteredSeasonContent.Add(type)) continue;
            ...
            {
                if (registerButton == null || !RegisteredSeasonContent.Add(type)) continue;
            ...
            {
                if (registerGameMode == null || !RegisteredSeasonContent.Add(type)) continue;
